...
We hereby proof that Dice loss is differentiable while IoU (Intersection over Union) loss is not. We begin by defining both loss functions and the used variables. Then, we reproduce the derivation in \yeartextcite{kervadec2023dice} for the gradients of the Dice loss and apply the same calculations to IoU.
